...
export class InformeSieteComponent implements OnInit {
  ...
  getFileInforme(file: File) {
    ...
    if (file.size > this.MAX_SIZE_FILE) {
      this.documento_informe7 = new FormData();
      Swal.fire({
        title: '¡Lo Sentimos!',
      ...
      if (result.value) {
        ...
        Swal.fire({
          ...
          onOpen:()=>{
            ...
            b.textContent = "Subiendo informe de la semana 7";
            this._pasantiaService.postDocumentoInf7(idEstudiante, this.documento_informe7).subscribe(async(resp:any)=> {
              if(resp){
                ...
                await this._notificacionService.sendInforme7Correo(this.info._id, notificacion).toPromise();
                Swal.fire({
                  title: '¡Bien Hecho!',
